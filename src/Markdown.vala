...
namespace ThiefMD.Enrichments {
    public class MarkdownEnrichment {
        ...
        public MarkdownEnrichment (string file) {
            ...
            try {
                ...
                var read_file = File.new_for_path (file);
                var input = new DataInputStream (read_file.read ());
                string line;
                while ((line = input.read_line (null)) != null) {
                    if (line.contains ("markdown:code-block")) {
                        string background = "background=\"";
                        if (line.contains (background)) {
                            code_block_background = line.substring (line.index_of (background, line.index_of ("markdown:code-block")) + background.length, 7);
                        }
                    }
                }
            } catch (Error e) {
